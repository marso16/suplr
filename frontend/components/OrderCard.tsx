  ...
  return (
    <Link href={`/dashboard/orders/${order.id}`}>
      <div className="group flex items-center gap-4 px-5 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:border-emerald-500/40 dark:hover:border-emerald-500/30 hover:shadow-sm transition-all cursor-pointer">
          ...
          <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">
            ...
            {order.items.length > 1 && (
              <span className="text-slate-400 dark:text-slate-500 font-normal">
                {" "}
                {t("more_items", { n: order.items.length - 1 })}
              </span>
            )}
          </p>
            ...
            {order.notes && (
              <>
                ...
                <svg
                  ...
                  stroke="currentColor"
                  strokeWidth={2}
                  className="inline text-slate-400 dark:text-slate-500"
                >
                  <path
                    strokeLinecap="round"
        ...
        <div
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${cfg.pill}`}
        >
          <div
