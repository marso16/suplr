...
function CreateSupplierModal({
  ...
  onClose: () => void;
  onCreated: (s: SupplierWithStats) => void;
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const created = (await api.admin.createSupplier(
        ...
        email,
        password,
      )) as unknown as SupplierWithStats;
      onCreated(created);
      onClose();
    } catch (err: unknown) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 border border-slate-700 text-slate-400 hover:text-slate-200 py-2.5 rounded-lg text-sm font-medium transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-violet-600 hover:bg-violet-700 text-white py-2.5 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer"
            >
              {loading ? (
                <>
                  <Spinner />
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-slate-900 border border-slate-700 rounded-xl w-full max-w-md p-6 shadow-2xl">
        ...
        {saved && (
          <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm px-3 py-2.5 rounded-lg mb-4">
            Password updated
          </div>
        )}
        <form onSubmit={handleSubmit} className="space-y-4">
          {[
            ...
            <div key={label} className="space-y-1.5">
              ...
              <input
                type="password"
                required
                minLength={label.includes("Current") ? 1 : 8}
                value={value}
                onChange={(e) => set(e.target.value)}
                placeholder="••••••••"
                className="w-full bg-slate-800 border border-slate-700 text-white placeholder:text-slate-500 px-3.5 py-2.5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/25 focus:border-violet-500 transition-colors"
...
export default function AdminDashboardPage() {
  ...
  async function impersonate(id: number) {
    ...
    try {
      ...
      window.open(
        `${BASE_URL}/dashboard?impersonate=${access_token}`,
        "_blank",
      );
    } finally {
      setBusy(null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-slate-900 border border-slate-700 rounded-xl w-full max-w-sm p-6 shadow-2xl">
            ...
            <p className="text-slate-400 text-sm mb-5">
              This will permanently delete the supplier and all their data —
              orders, clients, invoices, products. This cannot be undone.
            </p>
            <div className="flex gap-3">
              ...
              <button
                onClick={() => deleteSupplier(deleteConfirm)}
      ...
      <header className="border-b border-slate-800 px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="text-slate-400 hover:text-slate-200 text-sm transition-colors flex items-center gap-1.5 cursor-pointer"
          >
            <svg
              width="14"
              height="14"
                ...
                <div className="mb-5 flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">
                      ...
                      {realSuppliers.filter((s) => s.is_active).length} active
                      this week
                    </p>
                  </div>
                  <button
                    ...
                    <table className="w-full text-sm min-w-[800px]">
                      ...
                      <tbody className="divide-y divide-slate-800">
                        {realSuppliers.map((s) => (
                          <tr
                            key={s.id}
                            className={s.suspended ? "opacity-50" : ""}
                          >
                            ...
                            <td className="px-5 py-4">
                              {s.suspended ? (
                                ...
                                <span className="text-xs font-medium text-slate-400 bg-slate-500/10 px-2 py-0.5 rounded-full">
                                  Active
                                </span>
                              )}
                            </td>
            ...
            {tab === "orders" && (
              <>
                <div className="mb-5">
                  ...
                  <p className="text-slate-400 text-sm mt-1">
                    Last 200 orders across all suppliers
                  </p>
                </div>
                {orders.length === 0 ? (
                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden overflow-x-auto">
                    <table className="w-full text-sm min-w-[600px]">
                      <thead>
                        <tr className="border-b border-slate-800">
                          {["#", "Supplier", "Client", "Status", "Date"].map(
                            (h) => (
                              <th
                                key={h}
                                className="text-left px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide"
                        ...
                        {orders.map((o) => (
                          <tr key={o.id}>
                            ...
                            <td className="px-5 py-3.5">
                              <span
                                className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${STATUS_COLORS[o.status] ?? "text-slate-400 bg-slate-500/10"}`}
                              >
                                {o.status}
                              </span>
                            </td>
            ...
            {tab === "broadcast" && (
              <>
                <div className="mb-5">
                  ...
                  <p className="text-slate-400 text-sm mt-1">
                    Send an announcement to all active (non-suspended) suppliers
                  </p>
                </div>
                    ...
                    <form onSubmit={sendBroadcast} className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <textarea
                          ...
                          rows={6}
                          placeholder="Dear supplier, we'll be performing maintenance…"
