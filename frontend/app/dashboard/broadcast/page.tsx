...
export default function BroadcastPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{t("nav_broadcast")}</h1>
            ...
            {stage === "done" && result ? (
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-8 text-center space-y-4">
                <div className="w-12 h-12 rounded-full bg-emerald-100 dark:bg-emerald-500/10 flex items-center justify-center mx-auto">
                  <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} className="text-emerald-600 dark:text-emerald-400">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                    ...
                    {result.failed > 0 && (
                      <span className="text-red-500 dark:text-red-400"> · {result.failed} failed</span>
                    )}
                  </p>
                </div>
                <button
              ...
              <div className="bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-500/30 rounded-xl p-6 space-y-4">
                <div className="flex items-start gap-3">
                  <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="text-amber-500 flex-shrink-0 mt-0.5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                  </svg>
                  <div>
                    <p className="font-semibold text-slate-900 dark:text-slate-100 text-sm">
                      This will send to {clients.length} client{clients.length !== 1 ? "s" : ""}
                    </p>
                    <p className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">This action cannot be undone.</p>
                  </div>
                </div>
                ...
                {clients.map((c) => (
                  <div key={c.id} className="px-4 py-2.5 flex items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate leading-tight">{c.name}</p>
