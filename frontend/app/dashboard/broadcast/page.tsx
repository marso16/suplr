...
import { EmptyState, MegaphoneIllustration } from "@/components/EmptyState";
...
export default function BroadcastPage() {
  ...
  const canSend =
    (message.trim().length > 0 || !!mediaUrl) && clients.length > 0;
    ...
    <div className="h-full overflow-y-auto p-4 sm:p-6 lg:p-8">
      ...
      {clients.length === 0 ? (
        <EmptyState
          ...
          title="No clients yet — they appear once you receive a WhatsApp message."
        />
      ) : (
        <>
            ...
            <div className="flex items-center gap-0 mb-4 max-w-5xl">
              {STAGES.slice(0, 2).map((s, i) => (
                <div key={s} className="flex items-center gap-0">
                  ...
                  {i < 1 && (
                    <div
                      className={`w-8 h-px mx-2 ${stageIndex > i ? "bg-emerald-400" : "bg-slate-200 dark:bg-slate-700"}`}
                    />
                  )}
                </div>
              ))}
                  ...
                  <div className="flex flex-col items-center text-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                        Your message was dispatched to {result.total} recipient
                        {result.total !== 1 ? "s" : ""}
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 w-full max-w-xs">
                      <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 rounded-xl px-4 py-3 text-center">
                        ...
                        <p className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-500 uppercase tracking-wide mt-0.5">
                          Delivered
                        </p>
                      </div>
                      <div
                      ...
                      >
                        <p
                          className={`text-2xl font-bold tabular-nums ${result.failed > 0 ? "text-red-600 dark:text-red-400" : "text-slate-400 dark:text-slate-500"}`}
                        >
                          {result.failed}
                        </p>
                        <p
                          className={`text-[11px] font-semibold uppercase tracking-wide mt-0.5 ${result.failed > 0 ? "text-red-500 dark:text-red-500" : "text-slate-400 dark:text-slate-500"}`}
                        >
                          Failed
                        </p>
                      </div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 space-y-4">
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-xs tabular-nums ${message.length > 1000 ? "text-amber-500" : "text-slate-400 dark:text-slate-500"}`}
                      >
                  ...
                  <div>
                    <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wide mb-2">
                      ...
                      <span className="font-normal normal-case text-slate-400">
                        (optional — sent as WhatsApp image)
                      </span>
                    </p>
                    {mediaPreview ? (
