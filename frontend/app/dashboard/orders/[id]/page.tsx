  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <span className="font-mono text-sm font-semibold text-slate-400 dark:text-slate-500">#{order.id}</span>
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{t("order_detail")}</h1>
        <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${cfg.pill}`}>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 mb-4 grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
        <div>
          <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mb-1">{t("meta_client")}</p>
          <p className="text-slate-800 dark:text-slate-200 font-medium">{order.client.name}</p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5 font-mono">{order.client.whatsapp_number.replace(/@s\.whatsapp\.net$|@lid$/, "")}</p>
        </div>
        <div>
          <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mb-1">{t("meta_created")}</p>
          <p className="text-slate-800 dark:text-slate-200 font-mono text-[13px]">{new Date(order.created_at).toLocaleString()}</p>
        </div>
        {order.confirmed_at && (
          <div>
            <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mb-1">{t("meta_confirmed")}</p>
            <p className="text-slate-800 dark:text-slate-200 font-mono text-[13px]">{new Date(order.confirmed_at).toLocaleString()}</p>
          </div>
        )}
        <div>
          <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mb-1">{t("meta_currency")}</p>
          <p className="text-slate-800 dark:text-slate-200 font-medium">{order.currency}</p>
        </div>
        <div>
          <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mb-1">{t("meta_items")}</p>
          <p className="text-slate-800 dark:text-slate-200 font-medium">{order.items.length}</p>
        </div>
        <div className="col-span-2">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            {savingDate && (
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="animate-spin text-slate-400">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 dark:border-slate-800">
              <th className="text-start px-5 py-3 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">{t("col_product")}</th>
              <th className="text-end px-4 py-3 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">{t("col_qty")}</th>
              <th className="text-end px-4 py-3 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">{t("col_unit")}</th>
              <th className="text-end px-5 py-3 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">{t("col_price")}</th>
            </tr>
          </thead>
          <tbody>
            {order.items.map((item, i) => (
              <tr key={item.id} className={i > 0 ? "border-t border-slate-100 dark:border-slate-800" : ""}>
                <td className="px-5 py-3.5 text-slate-800 dark:text-slate-200">{item.product_name}</td>
                <td className="px-4 py-3.5 text-end font-mono text-slate-500 dark:text-slate-400">{item.quantity}</td>
                <td className="px-4 py-3.5 text-end text-slate-500 dark:text-slate-400">{item.unit}</td>
                <td className="px-5 py-3.5 text-end font-mono text-slate-800 dark:text-slate-200">{item.price}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
              ...
              <td className="px-5 py-3.5 text-end font-mono font-bold text-slate-900 dark:text-slate-100">
                {order.total} <span className="text-slate-400 dark:text-slate-500 font-normal text-xs">{order.currency}</span>
              </td>
            </tr>
          </tfoot>
        </table>
      ...
      <div className="flex gap-3">
        ...
        {order.status === "fulfilled" && (
          <button onClick={invoice} disabled={loading}
        ...
        {order.status === "invoiced" && (
          <span className="text-slate-400 dark:text-slate-500 text-sm py-2.5">{t("invoice_generated")}</span>
        )}
      </div>
    </div>
  );
