...
export class WhatsAppSenderService {
  ...
  async sendMessage(
    ...
    try {
      await axios.post(`${bspEndpoint.replace(/\/+$/, '')}/send`, payload, {
        headers: this.headers(bspApiKey),
        timeout: 10000,
      });
    } catch (e: any) {
  ...
  async sendDocument(
    ...
    try {
      await axios.post(
        `${bspEndpoint.replace(/\/+$/, '')}/send-document`,
        payload,
        { headers: this.headers(bspApiKey), timeout: 30000 },
      );
    } catch (e: any) {
  ...
  async enqueueBroadcast(
    ...
    try {
      const res = await axios.post(
        `${bspEndpoint.replace(/\/+$/, '')}/queue/broadcast`,
        payload,
        { headers: this.headers(bspApiKey), timeout: 10000 },
      );
