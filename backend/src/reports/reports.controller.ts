...
function bounds(period: string): [Date | null, Date] {
  const now = new Date();
  switch (period) {
    case '7d': return [new Date(Date.now() - 7 * 86400000), now];
    case '30d': return [new Date(Date.now() - 30 * 86400000), now];
    case '90d': return [new Date(Date.now() - 90 * 86400000), now];
    case '1y': return [new Date(Date.now() - 365 * 86400000), now];
    default: return [null, now];
  }
}
...
export class ReportsController {
  ...
  @Get()
  async getReport(@Query('period') period = '30d', @CurrentSupplier() supplier: Supplier): Promise<ReportResponse> {
    if (!/^(7d|30d|90d|1y|all)$/.test(period)) throw new Error('Invalid period');

    const cached = await this.cache.getCachedReport<ReportResponse>(supplier.id, period);
    ...
    const DAY_FMT = (d: Date) => d.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' });
    const MON_FMT = (d: Date) => d.toLocaleDateString('en-GB', { month: 'short', year: 'numeric' });
