...
interface SseEvent { type: string; order_id: number; }
...
export class SseService {
  ...
  constructor(private readonly cacheService: CacheService) {
    this.cacheService.sub.on('message', (channel: string, payload: string) => {
      ...
      if (subject) {
        try { subject.next(JSON.parse(payload) as SseEvent); } catch { /* ignore */ }
      }
    });
  }
  ...
  getSubject(supplierId: number): Subject<SseEvent> {
    if (!this.subjects.has(supplierId)) {
      ...
      this.cacheService.sub.subscribe(channel, (err: Error | null | undefined) => {
  ...
  async publishOrderEvent(supplierId: number, eventType: string, orderId: number): Promise<void> {
    const channel = `supplier:${supplierId}:orders`;
