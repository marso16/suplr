...
async def orders_ws(
    ...
    if not supplier or supplier.suspended:
        await websocket.close(code=4001)
        return
    ...
    try:
        await pubsub.subscribe(channel)
        logger.info("📡 WS opened for supplier %d", supplier.id)
        async for message in pubsub.listen():
            if message["type"] == "message":
                await websocket.send_text(message["data"])
    except WebSocketDisconnect:
        logger.info("🔌 WS closed for supplier %d", supplier.id)
    except Exception as e:
        logger.error("❌ WS error for supplier %d: %s", supplier.id, e)
    finally:
        await pubsub.unsubscribe()
        await redis.aclose()
